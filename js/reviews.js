 ...
 * RECENZII / REVIEWS
 * =====================================================================
 * Un singur modul, cu doi „furnizori" (providers), folosit pe orice
 * pagină de detaliu (locuri sau afaceri) care are hasReviews: true:
...
(function () {
  ...
  var LocalProvider = {
    key: function (entryId) { return "hl-reviews-" + entryId; },
    list: function (entryId) {
      try {
        var raw = localStorage.getItem(this.key(entryId));
        return raw ? JSON.parse(raw) : [];
      } catch (e) { return []; }
    },
    submit: function (entryId, data) {
      var all = this.list(entryId);
      all.unshift({
        rating: data.rating, author_name: data.name, title: data.title || "",
        body: data.body, image_url: data.imageDataUrl || "",
        created_at: new Date().toISOString()
      });
      try { localStorage.setItem(this.key(entryId), JSON.stringify(all.slice(0, 50))); } catch (e) {}
  ...
  var RemoteProvider = {
    list: function (entryId) {
      ...
      var url = base + "/rest/v1/reviews?select=rating,author_name,title,body,image_url,created_at" +
        "&place_id=eq." + encodeURIComponent(entryId) +
  ...
  function wireForm(form, entryId, onDone) {
  ...
  function mount(container, entryId) {
    if (!container) return;
    ...
    function render() {
      ...
      Promise.resolve(provider.list(entryId)).then(function (list) {
      ...
      wireForm(formWrap.querySelector("form"), entryId, function () {
        Promise.resolve(provider.list(entryId)).then(function (list) {
          list = Array.isArray(list) ? list : [];
